...
public class LinkedList<E extends Comparable<E>> {
	...
	public void removeAll(E el) {
		...
		first = null;
		removeAll(tempFirst, el);
	}

	private void removeAll(Node ref, E target) {
		if (ref == null) {
			return;
		}
		if (ref.data != target) {
			this.addLast(ref.data);
		}
		removeAll(ref.next, target);
	}
	...
	private void duplicateAll(Node ref, E target) {
		if (ref == null) {
			return;
		}
		if (ref.data == target) {
			Node store = ref.next;
			Node copy = new Node(ref.data);
			copy.next = store;
			ref.next = copy;
			duplicateAll(ref.next.next, target);
			n += 1;
		} else {
			duplicateAll(ref.next, target);
		}
	}
}